...
public class TransparentTilesOverlay extends SafeDrawOverlay
{
    ...
    public TransparentTilesOverlay(final MapTileLayerBase aTileProvider)
    {
        super();
    ...
    private final TileLooper mTileLooper = new TileLooper()
    {
                               ...
                               final MapTile pTile, final int pX, final int pY, final Rect pClipRect)
        {
            ...
            if (UtilConstants.DEBUGMODE)
            {
                ...
                canvas.drawText(pTile.toString(), mTileRect.left + 1, mTileRect.top + getDebugPaint().getTextSize(),
                        getDebugPaint());
    ...
    private SafePaint getLoadingTilePaint()
    {
        if (mLoadingTilePaint == null && mLoadingBackgroundColor != Color.TRANSPARENT)
        {
            try
            {
                ...
                mLoadingTilePaint.setShader(
                        new BitmapShader(mLoadingTileBitmap, Shader.TileMode.REPEAT, Shader.TileMode.REPEAT));
            }
                             ...
                             final Projection projection)
    {
        ...
        if (mTileProvider.hasNoSource() || Math.floor(pNewZoomLevel) == Math.floor(
                pOldZoomLevel) || projection == null || Math.abs(pOldZoomLevel - pNewZoomLevel) > mRescaleZoomDiffMax)
        {
            return;
        }
